...
export async function POST(req: NextRequest) {
  ...
  else return Response.json({ error: "Unsupported file" }, { status: 400 });
  ...
  return Response.json({
    ...
    avg_power, avg_hr, avg_speed_kmh, normalized_power: np, if: IF, tss: TSS,
    series
  });
}
...
async function parseFIT(buf: Buffer): Promise<Point[]> {
  ...
  return new Promise((resolve, reject) => {
    fit.parse(buf, (err: any, data: any) => {
      ...
      const t0 = rec[0]?.timestamp ? new Date(rec[0].timestamp).getTime() / 1000 : 0;
      const out = rec.map((r: any) => ({
        t: new Date(r.timestamp).getTime() / 1000 - t0,
        power: r.power, hr: r.heart_rate, speed_kmh: r.speed
      }));
      resolve(out);
    });
  });
...
async function parseTCX(buf: Buffer): Promise<Point[]> {
  ...
  for (const lap of laps) {
    ...
    for (const p of track) {
      ...
      points.push({ t: ts - t0, power: watts, hr });
    }
  }
  return points;
}
